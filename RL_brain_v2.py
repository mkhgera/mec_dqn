...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        def build_layers(s,lstm_s,c_names, n_l1, n_lstm, w_initializer, b_initializer):
            ...
            if self.dueling:
                ...
                with tf.compat.v1.variable_scope('Q'):
                    out = self.V + (self.A - tf.reduce_mean(input_tensor=self.A,axis=1,keepdims=True))  # Q = V(s) +A(s,a)
        ...
        # input for eval_net
        self.s = tf.compat.v1.placeholder(tf.float32,[None,self.n_features], name = 's')  # state (observation)
        self.lstm_s = tf.compat.v1.placeholder(tf.float32,[None,self.n_lstm_step,self.n_lstm_state], name='lstm1_s')

        self.q_target = tf.compat.v1.placeholder(tf.float32,[None,self.n_actions], name = 'Q_target') # q_target
        ...
        with tf.compat.v1.variable_scope('loss'):
            self.loss = tf.reduce_mean(input_tensor=tf.math.squared_difference(self.q_target,self.q_eval))
